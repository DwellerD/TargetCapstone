...
const wineTests = GroceryPage.wineSubcategoriesList.map(name => ({
  name,
  secureTitle: name
}));

describe('Wine Subcategory Secure Pages', () => {
  wineTests.forEach(({ name, secureTitle }) => {
    it(`should verify ${name}`, async () => {
      ...
      await GroceryPage.getItemByText('Wine, Beer & Liquor').click();
      // await browser.pause(1000);

      await GroceryPage.getItemByText('Wine').click();
      // await browser.pause(1000);
